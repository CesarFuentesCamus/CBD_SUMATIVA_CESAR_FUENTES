--***********************************************************
--SE ESTA TRABAJANDO CON EL USUARIO CREADO LA PRIMERA SEMANA:
...
DROP TABLE TRAMO_EDAD CASCADE CONSTRAINTS;
DROP TABLE  DETALLE_DE_CLIENTES;
...
VAR B_PERIODO VARCHAR2(6);
EXEC :B_PERIODO := '032024';

DECLARE
  -- Declaración de %ROWTYPE
  ...
  V_CONTADOR NUMBER := 0;
  V_EDAD NUMBER := 0; 
  V_COMUNA_VALIDO NUMBER; 
  V_PORCENTAJE NUMBER := 0; 

BEGIN
  -- Trunco la tabla de salida
  ...
  -- Uso un ciclo FOR
  FOR CLIENTE IN (
    SELECT c.id_cli, c.numrun_cli, c.appaterno_cli, c.apmaterno_cli, c.pnombre_cli, 
           c.renta, c.fecha_nac_cli, c.id_comuna, t.nombre_tipo_cli AS tipo_cliente
    FROM CLIENTE c
    JOIN TIPO_CLIENTE t ON c.id_tipo_cli = t.id_tipo_cli
    ORDER BY c.id_cli -- Garantizar el orden
  ) LOOP
    -- Incrementar el contador
    V_CONTADOR := V_CONTADOR + 1;

    -- Inicializar el registro
    R_DETALLE.IDC := CLIENTE.id_cli;
    R_DETALLE.RUT := CLIENTE.numrun_cli;
    R_DETALLE.CLIENTE := INITCAP(LOWER(CLIENTE.appaterno_cli)) || ' ' || 
                         INITCAP(LOWER(CLIENTE.apmaterno_cli)) || ' ' || 
                         INITCAP(LOWER(CLIENTE.pnombre_cli));

    -- Validar y calcular la edad del cliente
    IF CLIENTE.fecha_nac_cli IS NOT NULL THEN
      SELECT EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM CLIENTE.fecha_nac_cli) 
             - CASE 
                 WHEN TO_DATE(TO_CHAR(SYSDATE, 'MMDD'), 'MMDD') < 
                      TO_DATE(TO_CHAR(CLIENTE.fecha_nac_cli, 'MMDD'), 'MMDD') 
                 THEN 1 
                 ELSE 0 
               END 
      INTO V_EDAD
      FROM DUAL;
    ELSE
      V_EDAD := 0; -- Edad predeterminada si la fecha es NULL
    END IF;

    -- Insertar la edad calculada
    R_DETALLE.EDAD := V_EDAD;

    -- Verificar si la comuna está en la lista prohibida
    SELECT COUNT(*)
    INTO V_COMUNA_VALIDO
    FROM COMUNA 
    WHERE id_comuna = CLIENTE.id_comuna 
      AND nombre_comuna IN ('La Reina', 'Las Condes', 'Vitacura');

    -- **Calcular el puntaje según las reglas de negocio:**
    IF CLIENTE.renta > 700000 AND V_COMUNA_VALIDO = 0 THEN
      -- Regla 1: 3% de la renta
      R_DETALLE.PUNTAJE := ROUND(CLIENTE.renta * 0.03);
    ELSIF TRIM(UPPER(CLIENTE.tipo_cliente)) IN ('VIP', 'EXTRANJERO') THEN
      -- Regla 2: 30 puntos por año de edad
      R_DETALLE.PUNTAJE := V_EDAD * 30;
    ELSE
      BEGIN
        -- Regla 3: Obtener porcentaje de TRAMO_EDAD y aplicarlo a la renta
        SELECT porcentaje 
        INTO V_PORCENTAJE
        FROM TRAMO_EDAD
        WHERE TRAMO_INF <= V_EDAD AND V_EDAD <= TRAMO_SUP
        AND ROWNUM = 1; -- Garantiza que solo devuelva una fila

        R_DETALLE.PUNTAJE := ROUND(CLIENTE.renta * (V_PORCENTAJE / 100));
      EXCEPTION
        WHEN NO_DATA_FOUND THEN
          R_DETALLE.PUNTAJE := 0; -- Si no hay datos en TRAMO_EDAD
      END;
    END IF;

    -- Generar correo
    R_DETALLE.CORREO_CORP := LOWER(CLIENTE.appaterno_cli) || LPAD(V_EDAD, 2, '0') || '*' ||
                             SUBSTR(CLIENTE.pnombre_cli, 1, 1) ||
                             TO_CHAR(CLIENTE.fecha_nac_cli, 'DD') ||
                             SUBSTR(:B_PERIODO, 2, 1) || '@LogiCarg.cl';

    R_DETALLE.PERIODO := :B_PERIODO;

    -- Insertar registro en la tabla de salida
    INSERT INTO DETALLE_DE_CLIENTES VALUES R_DETALLE;

  END LOOP;

  -- Confirmar si se procesaron todos los clientes
  ...
  ELSE
    ROLLBACK;
    DBMS_OUTPUT.PUT_LINE('ERROR EN EL PROCESO. CAMBIOS DESHECHOS.');
  END IF;
END;
/
--visualización de tabla creada
SELECT * 
